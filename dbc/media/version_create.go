...
func (vc *VersionCreate) check() error {
	if _, ok := vc.mutation.Current(); !ok {
		return &ValidationError{Name: "Current", err: errors.New(`media: missing required field "Version.Current"`)}
	}
	if v, ok := vc.mutation.Current(); ok {
		if err := version.CurrentValidator(v); err != nil {
			return &ValidationError{Name: "Current", err: fmt.Errorf(`media: validator failed for field "Version.Current": %w`, err)}
		}
	}
	if _, ok := vc.mutation.Last(); !ok {
		return &ValidationError{Name: "Last", err: errors.New(`media: missing required field "Version.Last"`)}
	}
	if v, ok := vc.mutation.Last(); ok {
		if err := version.LastValidator(v); err != nil {
			return &ValidationError{Name: "Last", err: fmt.Errorf(`media: validator failed for field "Version.Last": %w`, err)}
		}
	}
	return nil
}
...
func (u *VersionUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("media: missing options for VersionCreate.OnConflict")
	}
...
func (u *VersionUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("media: OnConflict was set for builder %d. Set it on the VersionCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("media: missing options for VersionCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
