...
func (tx *Tx) init() {
	tx.Announce = NewAnnounceClient(tx.config)
	tx.Channel = NewChannelClient(tx.config)
	tx.Discovery = NewDiscoveryClient(tx.config)
	tx.InformationV1 = NewInformationV1Client(tx.config)
	tx.Page = NewPageClient(tx.config)
	tx.TopList = NewTopListClient(tx.config)
...
// applies a query, for example: Announce.QueryXXX(), the query will be executed
