...
func (ic *InstructCreate) check() error {
	if _, ok := ic.mutation.CurrentUnix(); !ok {
		return &ValidationError{Name: "current_unix", err: errors.New(`kernel: missing required field "innteInstruct.current_unix"`)}
	}
	if _, ok := ic.mutation.UpdatedUnix(); !ok {
		return &ValidationError{Name: "updated_unix", err: errors.New(`kernel: missing required field "innteInstruct.updated_unix"`)}
	}
	return nil
}
...
// of the `INSERT` statement. For example:
//
//	client.innteInstruct.CreateBulk(builders...).
//		OnConflict(
//			// Update the row with the new values
//			// the was proposed for insertion.
//			sql.ResolveWithNewValues(),
