...
func (vc *VersionCreate) check() error {
	if _, ok := vc.mutation.Current(); !ok {
		return &ValidationError{Name: "Current", err: errors.New(`kernel: missing required field "Version.Current"`)}
	}
	...
	if _, ok := vc.mutation.Last(); !ok {
		return &ValidationError{Name: "Last", err: errors.New(`kernel: missing required field "Version.Last"`)}
	}
...
func (u *VersionUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("kernel: missing options for VersionCreate.OnConflict")
	}
...
func (u *VersionUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("kernel: OnConflict was set for builder %d. Set it on the VersionCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("kernel: missing options for VersionCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
